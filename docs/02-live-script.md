...
마지막으로 요번달이죠. 12월엔 슬랙으로 사용하는 Claude Code가 나오고 
베타로 있던 Claude for Chrome, 크롬 확장자가 정식 출시 됐습니다.

이제 크롬에서 저 확장자만 설치하면 혼자 일을 해요. 아직은 쪼금 느린 감이 있지만 그래도 내가 워크플로우만 잘 설명해도 이전엔 수동으로 해야했던 일을 자동으로 처리합니다.

이것도 앞으로 중요한 내용이 될거에요.
Claude for Chrome은 내가 업무에 브라우징이 필요할 때만 사용하는 게 아니라,
Claude Code를 쓰는 중에도 같이 활용이 가능하거든요. 

그리고 Anthropic이 만들었던 MCP를 AAIF(Agentic AI Foundation) 재단에 기부를 하게 됐던 것도 12월의 이슈중 하나였어요.

...
4월엔 추론 모델인 **'o-series'**의 신규 모델들이 쏟아져 나왔습니다. 기존 모델보다 훨씬 빠르고 저렴해지면서, 앞서 인트로에서 말씀드린 '생각하는 AI'인 Reasoning Model이 일반 사용자들의 채팅창에도 기본값으로 들어오게 된 시점이었죠.

5월에는 애플의 전설적인 디자이너 조니 아이브가 설립한 **'io Products'**를 전격 인수합니다. 소프트웨어 회사인 OpenAI가 하드웨어 스타트업을 인수해서 이슈가 됐었어요. OpenAI가 "AI 시대에 맞는 새로운 디바이스"를 직접 만들겠다는 선언이기도 했죠. 동시에 **'Codex'**라는 코딩 에이전트의 프리뷰도 공개했는데요. 이때가 Anthropic이 Claude Code를 정식 출시했던 시기에요. Claude Code는 CLI, 로컬 환경에서 사용하는 형태였는데, 대조적으로 Codex는 클라우드 환경에서 사용하는 형태로 나왔습니다.

7월은 사용자 경험(UX) 측면에서 큰 변화가 있었습니다. 1월에 나온 Operator가 ChatGPT 안으로 완전히 통합됐거든요. 이때부터 채팅창에 **'글쓰기 모드(Chat)'**와 '일하기 모드(Agent)' 스위치가 생겼죠. 그냥 대화할 땐 Chat, 무언가 실행을 시킬 땐 Agent. 사용자는 복잡한 설치 없이 ChatGPT 앱 하나면 모든 게 가능해졌습니다.

그리고 뜨거운 여름, 8월에 드디어 그 모델이 나왔습니다. GPT-5. 작년부터 온다 온다 하던 그 모델이 출시되면서 다시 한번 성능의 격차를 벌렸습니다. "이게 AGI(일반인공지능)냐 아니냐"로 한창 시끌시끌했었죠. 이때 개발자용 도구인 Codex CLI도 같이 출시하면서, 앤스로픽의 텃밭인 개발자 생태계에도 본격적인 공세를 시작했습니다.

9월엔 Sora 2가 출시됐는데, 이번엔 단순한 영상 생성 웹사이트가 아니었습니다. iOS 앱으로 나오면서 마치 틱톡이나 릴스처럼 변했어요. 사람들이 텍스트로 영상을 만들고 바로 공유하는 소셜 미디어가 된 거죠. 창작의 도구를 넘어선 **'놀이의 도구'**가 된 겁니다.

10월엔 아예 **'ChatGPT Atlas'**라는 브라우저를 공개해버립니다. 크롬 확장 프로그램 수준이 아니라, 그냥 브라우저 자체를 만든 건데요. 검색부터 웹 서핑, 요약, 쇼핑까지 브라우저 자체가 거대한 에이전트가 되는 경험을 제공했습니다.

연말인 12월에는 GPT 모델의 마이너 업데이트와 함께 재미있는 행보가 보입니다. 바로 'Codex Skills' 도입인데요. 앞서 앤스로픽 파트에서 말씀드린 Skills 개념 기억나시나요? 앤스로픽이 만든 그 표준 규격을 OpenAI도 그대로 채택했습니다. 경쟁사지만 "좋은 건 갖다 쓰자"는 거죠. 덕분에 개발자들은 SKILL.md 파일 하나만 만들면 클로드에서도, GPT에서도 똑같이 동작하는 에이전트를 만들 수 있게 됐습니다. 이게 바로 **'생태계 표준'**이 만들어지는 과정이죠.

마지막으로 디즈니와의 파트너십을 통해 Sora로 만든 콘텐츠가 실제 메이저 미디어 시장에 진출하는 그림까지 보여주며 한 해를 마무리했습니다.

OpenAI의 2025년을 보면 확실히 **'일상의 넓이'**가 느껴지시나요? 우리가 검색하고, 쇼핑하고, 영상을 보고, 코딩하는 그 모든 순간순간에 OpenAI의 제품들이 스며들었던 한 해였습니다.
